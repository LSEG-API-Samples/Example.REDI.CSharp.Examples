...
{
    class VerticalStrategy : IStrategy
    {
        protected Options options;

        protected string name = "Vertical";
        ...
        public bool VerifyArguments()
        {
            ...
            if(string.Equals(options.Side1, options.Side2, StringComparison.OrdinalIgnoreCase))
            {
                ret = false;
                Console.WriteLine("Side of the first leg ({0}) couldn't be equal to the side of the second leg ({1})", options.Side1, options.Side2);
            }
            return ret;
        }
